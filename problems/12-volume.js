...
// calc volume
// measure ht, wd, and l one at a time
// 

const recVolume = height => {
  let variableCount = [height];
  
  //let extraStuff = function (variable) {
  const extraStuff = (num) => {
    if (variableCount.length < 3) {
      variableCount.push(num);
      }
    if (variableCount.length === 3) {
    // let volumeCalc = variableCount.reduce(accum, currentValue)
      let volumeCalc = variableCount.reduce((acc, el) => (acc *= el));
      return volumeCalc;
  } else {
      return extraStuff;
  }
